...
public class Company {
	...
	enum Department {IT, SALES, MARKETING};
	...
	private void setName(String name) {
		if (name.length() < 2) {
			throw new IllegalArgumentException("Company name must be at least 2 symbols.");
		}
	...
	void printEmployeesBySalary(){
		for (Department d : workers.keySet()) {
			Collections.sort(workers.get(d), new Comparator<Employee>() {
				@Override
				public int compare(Employee o1, Employee o2) {
					int compareBySalary = (int)(o1.getSalary() - o2.getSalary());
